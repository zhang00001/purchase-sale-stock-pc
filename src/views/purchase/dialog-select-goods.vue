<template>
  <div>
    ...
    >
      ...
      <el-table-column key="15" prop="disprice" label="折后价" align="center"></el-table-column>
      <el-table-column key="16" prop="money" label="金额" align="center"></el-table-column>
      <el-table-column key="17" prop="dismoney" label="折后金额" align="center"></el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    saveMsg() {
      saleMessage({ id: this.$route.query.id, content: this.content }).then(
        res => {
          this.getMsg(1);
          this.$meaaage.success(res.msg);
        }
    ...
    // 留言，列表
    getMsg(page) {
      saleMessageGet(this.$route.query.id).then(res => {
    ...
    checkNum(item, cum) {
      ...
      goods.forEach(val => {
        if (val.id == item.id) {
          ...
          val.disprice = item.disprice;
          val.money = item.money;
          val.dismoney = item.dismoney;
        }
      });
    ...
    saveLocal() {
      ...
      if (
        ...
        this.$store.commit(
          "good",
          this.$store.state.good
            .filter(val => val.goodId != this.selectGood.id)
            .concat(this.goodons)
        );
      } else {
    ...
    selsctGood(e) {
      ...
      goodsOne(e.id).then(res => {
        ...
        this.selectGood = e;
        this.goodons = [];
        console.log(this.$store.state.good.filter(val => val.goodId == e.id));
        //  缓存中是否含有当前选中的商品
