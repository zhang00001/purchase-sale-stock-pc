      ...
      <el-table :data="tableData" border>
        <el-table-column label="操作" width="100" align="center">
          <template slot-scope="scope">
            <el-button @click="look(scope.row.status2, scope.row.id, scope.row.type)" type="text" size="small">查看</el-button>
          </template>
        </el-table-column>
...
<script>
import { enumOut } from "@/utils/enums"
import { customerCategoryList } from "@/api/index.js"
import { MakerList, AllStore, warehouseList, StorageListList } from "@/api/api.js"
  ...
  methods: {
    ...
    look(status, id, type) {
      this.$router.push({
        ...
        query: { status: status, id: id, type: type }
      })
    },
