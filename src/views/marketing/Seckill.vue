        ...
        <el-card class="box-card">
          <div class="top">
            <div style="margin:0 60px 0 20px;">
              ...
              <el-button type="primary" @click="dialogVisible = true">新增秒杀</el-button>
            </div>
          </div>
          ...
          <el-table :data="tableData" border style="width: 100%">
            <el-table-column fixed label="操作" width="200">
              <template slot-scope="scope">
                ...
                <el-button
                  v-if="status==1"
                  @click="Goonline(scope.row,1)"
                  type="text"
                  size="small"
                >上线</el-button>
                <el-button
                  v-if="status==2||status==2"
                  @click="cel(scope.row)"
                  type="text"
                  size="small"
                >停止</el-button>
            ...
            <el-table-column prop="num" label="限购数量"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="dialogVisible" :before-close="clearForm">
      <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="运费" prop="fare">
          <el-input v-model="ruleForm.fare" v-if="type==0||type==1"></el-input>
          <span v-else>{{ruleForm.fare}}</span>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async handleClick(e, type) {
      ...
      let res = await seckillInfo(e.id);
      if (res) {
    ...
    async saveDiscount() {
      let res;
      if (this.ruleForm.id) {
        res = await seckillEdit(this.ruleForm.id, {
          ...this.ruleForm,
          goods: this.$refs.discountgood.goodTable
        });
      } else {
        res = await seckillAdd({
          ...this.ruleForm,
          goods: this.$refs.discountgood.goodTable
        });
      }
    ...
    async getTableDaTa(page) {
      let res = await seckill({ status: this.status, page: page, size: 10 });
