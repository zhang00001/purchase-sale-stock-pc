          ...
          <div class="box-card">
            ...
            <div class="right">
              <div class="center">
                ...
                <el-button type="primary" @click="dialogFormVisible2=true">新增颜色</el-button>
              </div>
              <div class="box">
    ...
    >
      <el-form :model="form" label-width="auto" ref="form">
        <el-form-item
          label="颜色名称"
          prop="name"
          :rules="[{ required: true, message: '请输入颜色名称', trigger: 'blur' }]"
        >
        ...
        <el-form-item label="默认颜色" prop="default">
          <el-select v-model="form.default" placeholder="是否默认颜色">
  ...
  methods: {
    ...
    saveColor() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id) {
            ColorEdit(this.form.id, this.form).then(res => {
              if (res.code == 200) {
                this.$message.success(res.msg);
    ...
    getColorGroup(page) {
      ColorGroupList({ params: { page: page, size: 10 } }).then(res => {
        if (res.code == 200) {
    ...
    saveGroup() {
      if (this.gname == "") {
        ...
        if (this.selectData) {
          ColorEditGroup(this.selectData.gid, {
            name: this.gname
          }).then(res => {
            if (res.code == 200) {
              this.$message.success(res.msg);
          ...
          ColorAddGroup({ name: this.gname }).then(res => {
            if (res.code == 200) {
              this.$message.success(res.msg);
              this.getColorGroup(1);
              this.cel();
